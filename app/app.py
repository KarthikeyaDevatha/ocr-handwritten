...
def main():
    ...
    mode_labels = {"auto": "auto", "trocr": "trocr", "mathpix": "Texteller", "arithmetic": "Paddle OCR"}
    mode = st.sidebar.selectbox(
        "OCR Mode",
        ["auto", "trocr", "mathpix", "arithmetic"],
        index=0,
        format_func=lambda x: mode_labels.get(x, x),
        help="Auto: smart routing | TrOCR: local offline | Texteller: Math/STEM OCR | Paddle OCR: general OCR"
    )
                ...
                with st.spinner(f"Processing with mode: {mode.upper()}..."):
                    ...
                    try:
                        ...
                        if result.mathpix_result:
                            st.subheader("🔬 Texteller Details")
    ...
    with tab_compare:
        ...
        if compare_file:
            ...
            st.image(comp_image, caption="Comparison Image", width='stretch')

            engine_labels = {"trocr": "trocr", "mathpix": "Texteller", "arithmetic": "Paddle OCR"}
            ...
            if st.button("⚡ Run Comparison", type="primary"):
                ...
                try:
                    ...
                    for i, engine in enumerate(engines_to_compare):
                        with cols[i]:
                            engine_disp = engine_labels.get(engine, engine.upper())
                            st.subheader(f"🔧 {engine_disp.upper() if engine_disp == engine else engine_disp}")
                            with st.spinner(f"Running {engine_disp}..."):
    ...
    | **Texteller** | Math/STEM (local) |
    | **Paddle OCR** | General/Printed text OCR |
    """)
    st.sidebar.markdown("---")
    st.sidebar.caption("Adaptive OCR Agent v2.0 • TrOCR + Texteller + CRAFT")
